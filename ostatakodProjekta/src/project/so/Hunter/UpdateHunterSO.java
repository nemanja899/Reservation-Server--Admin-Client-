 ...
 * @author User
 */
public class UpdateHunterSO extends AbstractSO {

    private final Repository hunterStorrage;

    public UpdateHunterSO() {
        hunterStorrage = new HunterRepository();
    }

    @Override
    protected void precondition(Object entity) throws ValidationException {
        if (!(new HunterRepository().updateFirstObject(entity) instanceof Hunter) || !(new HunterRepository().updateSecondPrimitive(entity) instanceof String) ) {
            throw new ValidationException("Padaci nisu validni");
        }
    }
    ...
    protected Object executeOperation(Object entity, Connection conn) throws Exception {
        try {
             hunterStorrage.eddit(new HunterRepository().updateFirstObject(entity),new HunterRepository().updateSecondPrimitive(entity), conn);
             return true;
        } catch (Exception e) {
            e.printStackTrace();
            throw new Exception("Greska u pronalezenju lovca");
        }
    }
